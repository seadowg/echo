\chapter{Functional Reactive Programming}
  ...
  \section{Semantics}
    ...
    Behaviours and Events. Here the semantics of `classic' \footnote{Several
    ...
    \subsection{Types}
      \subsubsection{Time}
        ...
        Time as a continuous value and so in in numerical terms this is best modelled by a the
        Scala type Double and this is the definition we will use throughout this report:
...
\end{verbatim}    
        ...
        conceptually a there is no minimum Double value but with Time we can say that a given Time is `at least'
...
\end{verbatim}
       ...
       slowing down or speeding up a Behaviour. We define this with the timeTransform as such:
...
\end{verbatim}            
      ...
      the occurrence from ev2 will be placed after in the merged stream. Another interesting point to explore
      here is that often we can't evaluate which Event occurred first. Imagine one Event has one occurrence in
      the future but the other has no occurrence: the second still might occur before the first, we just don't know it
