\chapter{Implementation}
  ...
  as powerful as possible. They will all be explored here.
    ...
    \subsection{Flat Time}
      % Many implementations use non flat time
      % Scala currently doesn't provide value from non-flat types
      % No non-flat features needed externally to the framework
      % Time implemented as Double allows switching between them implicitly
...
\end{verbatim}       
    ...
    \subsection{Extendable Events}
      % Events are often used to model external stimuli
      % Events need to actually be able to occur
      % EventSource allows extension for occurable Events
      % Other Events modeled as Event - simply lazy views of some EventSource
